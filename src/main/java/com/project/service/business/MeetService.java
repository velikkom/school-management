...
public class MeetService
{
    ...
    public ResponseEntity<Page<MeetResponse>> getAllMeetByTeacher(HttpServletRequest httpServletRequest, int page, int size)
    {
        ...
        pageableHelper.getPageableWithProperties(page, size);

        return ResponseEntity.ok(meetRepository.findByAdvisoryTeacher_IdEquals(advisoryTeacher.getId(), pageableHelper.getPageableWithProperties(page, size))
                .map(meetMapper::mapMeetToMeetResponse));
